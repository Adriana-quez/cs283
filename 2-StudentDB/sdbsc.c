...
int get_student(int fd, int id, student_t *s)
{
    ...
    int readStudent = read(fd, s, STUDENT_RECORD_SIZE);
    if (readStudent == -1) {
        return ERR_DB_FILE;
    }

    if (memcmp(s, &EMPTY_STUDENT_RECORD, readStudent) == 0) {
...
int add_student(int fd, int id, char *fname, char *lname, int gpa)
{
    ...
    int readStudent = read(fd, &student, STUDENT_RECORD_SIZE);
    if (readStudent == -1) {
        printf(M_ERR_DB_READ);
        return ERR_DB_FILE;
    }

    if (memcmp(&student, &EMPTY_STUDENT_RECORD, readStudent) == 0) {
        if (write(fd, &student, STUDENT_RECORD_SIZE) == -1) {
...
int count_db_records(int fd)
{
    ...
    while ((readStudent = read(fd, &student, STUDENT_RECORD_SIZE)) > 0) {
        if (memcmp(&student, &EMPTY_STUDENT_RECORD, readStudent) != 0) {
            ++studentCount;
        }
    }
...
int print_db(int fd)
{
    ...
    while ((readStudent = read(fd, &student, STUDENT_RECORD_SIZE)) > 0) {
        if (memcmp(&student, &EMPTY_STUDENT_RECORD, readStudent) != 0) {
            float calculateGPA = student.gpa / 100.0;
            printf(STUDENT_PRINT_FMT_STRING, student.id, student.fname, student.lname, calculateGPA);
        }
    }
...
void print_student(student_t *s)
{
    if (s != NULL && s->id != 0) {
        ...
        float calculateGPA = s->gpa / 100.0;
        printf(STUDENT_PRINT_FMT_STRING, s->id, s->fname, s->lname, calculateGPA);
    } else {
